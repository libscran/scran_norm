...
TEST(CenterSizeFactors, Simple) {
    ...
    scran::center_size_factors::Options opt;

    scran::sanitize_size_factors::Diagnostics diag;
    auto copy = sf;
    auto out = scran::center_size_factors::compute(copy.size(), copy.data(), &diag, opt);
    ...
    auto copy2 = sf;
    auto out2 = scran::center_size_factors::compute(copy2.size(), copy2.data(), &diag, opt);
    EXPECT_EQ(out, out2);
...
TEST(CenterSizeFactors, IgnoreZeros) {
    ...
    auto sf1 = sf;
    scran::sanitize_size_factors::Diagnostics diag;
    auto out = scran::center_size_factors::compute(sf1.size(), sf1.data(), &diag, opt);
    ...
    {
        auto sf2 = sf;
        auto out2 = scran::center_size_factors::compute(sf2.size() - 1, sf2.data() + 1, NULL, opt);
        EXPECT_EQ(out, out2);
        EXPECT_EQ(sf1, sf2);
    }
    ...
    // Restoring our acceptance of zeros.
    {
        auto sf3 = sf;
        opt.ignore_invalid = false;
        auto out3 = scran::center_size_factors::compute(sf3.size(), sf3.data(), NULL, opt);
        EXPECT_LT(out3, out);
    }
    ...
    // All-zeros avoid division by zero.
    {
        ...
        scran::center_size_factors::Options opt;
        auto copy = empty;
        auto out4 = scran::center_size_factors::compute(copy.size(), copy.data(), NULL, opt);
        EXPECT_EQ(out4, 0);
...
TEST(CenterSizeFactors, Blocked) {
    ...
    scran::sanitize_size_factors::Diagnostics diag;
    auto sf1 = sf;
    auto out = scran::center_size_factors::compute_blocked(sf1.size(), sf1.data(), block.data(), &diag, opt);
    ...
    opt.block_mode = scran::center_size_factors::BlockMode::PER_BLOCK;
    auto sf2 = sf;
    auto out2 = scran::center_size_factors::compute_blocked(sf2.size(), sf2.data(), block.data(), NULL, opt);
    EXPECT_EQ(out, out2);
...
TEST(CenterSizeFactors, BlockIgnoreZeros) {
    ...
    // Works in the other mode.
    {
        ...
        opt.block_mode = scran::center_size_factors::BlockMode::PER_BLOCK;
        auto sf1 = sf;
        auto out = scran::center_size_factors::compute_blocked(sf1.size(), sf1.data(), block.data(), NULL, opt);
    ...
    {
        auto sf2 = sf;
        auto out2 = scran::center_size_factors::compute_blocked(sf2.size() -1, sf2.data() + 1, block.data() + 1, NULL, opt);
        EXPECT_EQ(out, out2);
        EXPECT_EQ(sf1, sf2);
    }
    ...
    // We can handle all-zero groups.
    {
        ...
        scran::center_size_factors::Options opt;
        auto out = scran::center_size_factors::compute_blocked(empty.size(), empty.data(), block.data(), NULL, opt);
        EXPECT_EQ(out, std::vector<double>(2));
        EXPECT_EQ(empty, std::vector<double>(empty.size()));

        opt.block_mode = scran::center_size_factors::BlockMode::PER_BLOCK;
        auto out2 = scran::center_size_factors::compute_blocked(empty.size(), empty.data(), block.data(), NULL, opt);
